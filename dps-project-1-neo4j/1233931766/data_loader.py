...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        with self.driver.session(database="neo4j") as session:
            print("Creating constraint on Location.name...")
            session.run(
                """CREATE CONSTRAINT location_name_unique IF NOT EXISTS FOR (l:Location) REQUIRE l.name IS UNIQUE"""
            )

            # CSV name is relative to /var/lib/neo4j/import
            csv_filename = file_path.split(".")[0] + ".csv"
            csv_url = f"file:///{csv_filename}"
            print(f"Loading data from {csv_filename} into Neo4j...")

            load_query = """
            CALL {
              LOAD CSV WITH HEADERS FROM $filename AS row
              WITH row
              WHERE row.PULocationID <> '' AND row.DOLocationID <> ''
              MERGE (pu:Location {name: toInteger(row.PULocationID)})
              MERGE (do:Location {name: toInteger(row.DOLocationID)})
              CREATE (pu)-[:TRIP {
            ...
            """
            session.run(load_query, filename=csv_url)
